...
namespace pg
{
    ...
    Game::Game(int vcount, int ecount) : _owner(vcount), solved(vcount), winner(vcount)
    {
        assert(vcount > 0);
        if (ecount == -1)
            ecount = size_t(4) * vcount; // reasonable default outdegree
    ...
    Game::Game(const Game &other) : Game(other.n_vertices, other.e_size)
    {
        ...
        {
            if (other._label[i])
                _label[i] = new std::string(*other._label[i]);
        }
    ...
    Game::set_priority(int node, int priority)
    {
        ...
        if (is_ordered)
        {
            if (node > 0 and _priority[node - 1] > _priority[node])
                is_ordered = false;
    ...
    {
        if (this->_label[node])
            delete this->_label[node];
        if (label != "")
            this->_label[node] = new std::string(label);
        else
            this->_label[node] = 0;
    }
    ...
    Game::parse_pgsolver(std::istream &inp, bool removeBadLoops)
    {
        ...
        inp.read(buf, 6);
        if (!inp)
            throw "expecting parity game specification";
        if (strncmp(buf, "parity", 6) != 0)
            throw "expecting parity game specification";

        skip_whitespace(rd);
        if (!read_uint64(rd, &n))
            throw "missing number of nodes";
        ...
        while (node_count < n_vertices)
        {
            ...
            if (n > INT_MAX)
                throw "priority too high"; // don't be ridiculous
            _priority[id] = n;
            ...
            for (;;)
            {
                ...
                skip_whitespace(rd);
                if (!(inp >> ch))
                    throw "missing ; to end line";
                if (ch == ',')
                    continue; // next successor
                if (ch == ';')
                    break; // end of line
                if (ch == '\"')
                {
    ...
    Game::parse_solution(std::istream &in)
    {
        ...
        while (getline(in, line))
        {
            ...
            if (solved[ident])
                throw "node already solved";

            // parse winner
            int w;
            if (!(ss >> w))
                throw "missing winner";
            if (w != 0 && w != 1)
                throw "invalid winner";
            ...
            if (w == _owner[ident])
            {
                int str;
                if (!(ss >> str))
                    throw "missing strategy for winning node";
    ...
    Game::write_pgsolver(std::ostream &os)
    {
        ...
        for (int i = 0; i < n_vertices; i++)
        {
            ...
            if (_label[i] != 0 and !_label[i]->empty())
                os << " \"" << *_label[i] << "\"";
            os << ";" << std::endl;
        }
    }
    ...
    Game::write_sol(std::ostream &out)
    {
        ...
        for (int i = 0; i < n_vertices; i++)
        {
            ...
            {
                out << i << " " << (winner[i] ? "1" : "0");
                if (winner[i] == _owner[i] and strategy[i] != -1)
                    out << " " << strategy[i];
                out << ";" << std::endl;
            }
        }
    ...
    Game::sort(int *mapping)
    {
        ...
        else
        {
            ...
            // sort the mapping
            std::sort(mapping, mapping + n_vertices, [&](const int &a, const int &b)
                      { return (unsigned int)priority(a) < (unsigned int)priority(b); });
    ...
    Game::unsafe_permute(int *mapping)
    {
        ...
        {
            if (strategy[i] != -1)
                strategy[i] = mapping[strategy[i]];
        }
        unsigned long len = n_vertices + n_edges;
        for (unsigned long i = 0; i < len; i++)
        {
            if (_outedges[i] != -1)
                _outedges[i] = mapping[_outedges[i]];
        }
        if (_inedges != NULL)
        {
            for (unsigned long i = 0; i < len; i++)
    ...
    Game::extract_subgame(bitset mask)
    {
        ...
        for (int v = 0; v < n_vertices; v++)
        {
            ...
            if (_label[v] != 0)
                ...
                res->init_vertex(w, _priority[v], _owner[v], "");
        }
